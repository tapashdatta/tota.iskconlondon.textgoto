 ...
 *
 */
class tota_iskconlondon_textgoto extends CRM_SMS_Provider {
  ...
  /**
   * textgoto API Server Session ID
   *
   * @var string
   */
   ...
   * Constructor
   *
   * Create and auth a textgoto session.
   *
   * @return void
   */
  ...
  static function &singleton($providerParams = array(
    ...
    if (!isset(self::$_singleton[$cacheKey]) || $force) {
      ...
      self::$_singleton[$cacheKey] = new tota_iskconlondon_textgoto($provider, $skipAuth);
    }
    return self::$_singleton[$cacheKey];
  }

  /**
   * Authenticate to the textgoto API Server.
   *
...
//      'api_id'    => $this->_providerInfo['api_params']['api_id']
  ...
  /**
   * Send an SMS Message via the textgoto API Server
   *
  ...
  function send($recipients, $header, $message, $jobID = NULL) {
    if ($this->_apiType = 'http') {
       ...
       * with textgoto is assigned three queues namely 1, 2 and 3.
       */
  ...
  function curl($url, $postData) {
    ...
    elseif ($response['http_code'] === 0) {
      return PEAR::raiseError('Cannot connect to the textgoto API Server.');
    }
